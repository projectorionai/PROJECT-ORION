...
> **This is a point-in-time snapshot, not a living document.** The check that
> actually runs is `tests/test_requirements_honesty.py`: it reads
> `requirements.txt`, resolves each package's real module names from installed
> distribution metadata, and fails if a package is neither imported anywhere nor
> accompanied by a comment explaining why it is installed. It also fails if a
> heavy package (torch, sklearn, matplotlib, mediapipe, resemblyzer, whisper)
> reappears at module scope, which would silently undo ORION's 466 ms startup.
...
| matplotlib | **test fixture only** — builds a real PDF for the flashcard reader test. ORION never imports it; `reports.py` renders charts as pure-Python inline SVG. | |
