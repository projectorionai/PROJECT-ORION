...
so the GUI can follow rendering without ever touching a worker.
...
class AudioStudioService:
        ...
        try:
            if self._ffmpeg is not None:
                ...
                if source.suffix.lower() != ".wav" or convert_to != "wav":
                    ...
                    return ToolResult(
                        f"Without ffmpeg I can only normalise 16-bit WAV to WAV. "
    ...
    def _export_package(self, name: str) -> ToolResult:
        ...
        if not processed:
            return ToolResult("There are no processed stems to package yet.", ok=False)
        ...
        for record in processed:
            ...
            try:
                ...
                continue
        atomic_write_text((folder / "manifest.json"),
        ...
        self._activity("package", {"package": slug, "count": len(manifest),
                                   "folder": str(folder)})
        return ToolResult(
            f"Exported {len(manifest)} stem(s) to package '{folder.name}' "
