...
Contextual naming (Mark XX architectural-audit pass, Track I): a monitor's
raw ``name`` is an OS device string (``\\\\.\\DISPLAY1``) or a bare index —
useless in speech or a status line. ``contextual_name`` resolves a
human-meaningful label instead: a user-set name when one has been given
(via ``set_custom_name``, persisted to config), else an objectively
geometry-derived role (Vertical / Primary / Secondary) rather than a guess
at what the monitor is semantically FOR — ORION cannot know a monitor is
"the gaming monitor" without being told, so it asks to be told rather than
hallucinating it.
"""
...
_DISPLAY_NAMES_PATH = CONFIG_DIR / "display_names.json"
...
class DisplayTopologyManager:
    ...
    def __init__(self, telemetry: Any | None = None) -> None:
        ...
        # Last logged topology signature — refresh() is called every few
        # seconds by cached-topology consumers, so it only LOGS when the
        # physical arrangement actually changes (hot-plug, resolution, DPI).
        self._last_signature: tuple = ()
        self._custom_names: dict[str, str] = self._load_custom_names()
    ...
    def set_custom_name(self, device: str, label: str) -> ToolResult:
        """Persist a human-chosen name for a monitor (matched by its raw OS
        device string, e.g. \\\\.\\DISPLAY1) — the only honest way to learn
        what a monitor is FOR, since geometry alone can't say "gaming"."""
        device = str(device or "").strip()
        label = str(label or "").strip()
        if not device or not label:
            return ToolResult("Both a device and a label are required.", ok=False)
        self._custom_names[device] = label
        try:
            CONFIG_DIR.mkdir(parents=True, exist_ok=True)
            atomic_write_text(_DISPLAY_NAMES_PATH,
                json.dumps(self._custom_names, indent=2), encoding="utf-8")
        except OSError:
            pass  # a read-only config directory must never break the call
        return ToolResult(f"'{device}' will now be called '{label}'.")

    def contextual_name(self, monitor: "Monitor") -> str:
        """A human-meaningful label: the user's own name if one was set for
        this device, else an objective geometry-derived role."""
        custom = self._custom_names.get(monitor.name)
        if custom:
            return custom
        if monitor.height > monitor.width:
            return "Vertical Monitor"
    ...
    def refresh(self) -> Topology:
        ...
        signature = tuple(
            (m.x, m.y, m.width, m.height, m.is_primary, round(m.scale, 3), m.name)
            for m in monitors
        )
    ...
    def describe(self) -> dict[str, Any]:
        ...
        return {
            "monitors": [
                {**m.to_dict(), "contextual_name": self.contextual_name(m)}
    ...
    def summary(self) -> str:
        ...
        for m in self.topology().monitors:
            ...
            parts.append(
                f"{flag}[{m.index}] {self.contextual_name(m)} {m.width}x{m.height} "
                f"@ ({m.x},{m.y}) scale {int(m.scale * 100)}%"
            )
