...
class EmotionProfile:
    ...
    palette_dark: tuple[int, int, int] = (30, 6, 10)
    palette_mid: tuple[int, int, int] = (150, 22, 42)
    palette_bright: tuple[int, int, int] = (255, 106, 128)
    accent: tuple[int, int, int] = (255, 42, 68)

    def params(self) -> dict[str, Any]:
        return asdict(self)


# ORION is CRIMSON-branded: every emotion stays inside a warm red family, so
# the face always reads as "ORION red" while brightness / saturation / warmth
# still shift to express feeling.  The variable names below are historical
# (they read as hues but now hold red-family tones keyed by intensity):
#   _BLUE→calm crimson · _BRIGHT_BLUE→bright rose · _DEEP_BLUE→deep crimson
#   _AMBER→warm ember · _AMBER_RED→hot red · _RED→critical red · _VIOLET→muted red
_BLUE = dict(palette_dark=(30, 6, 10), palette_mid=(150, 22, 42),
             palette_bright=(255, 106, 128), accent=(255, 42, 68))
_BRIGHT_BLUE = dict(palette_dark=(42, 10, 16), palette_mid=(205, 40, 66),
                    palette_bright=(255, 150, 165), accent=(255, 84, 118))
_DEEP_BLUE = dict(palette_dark=(20, 4, 8), palette_mid=(112, 16, 34),
                  palette_bright=(224, 72, 92), accent=(232, 34, 60))
_AMBER = dict(palette_dark=(34, 12, 6), palette_mid=(170, 70, 28),
              palette_bright=(255, 168, 96), accent=(255, 120, 52))
_AMBER_RED = dict(palette_dark=(40, 8, 6), palette_mid=(196, 44, 22),
                  palette_bright=(255, 128, 84), accent=(255, 78, 44))
_RED = dict(palette_dark=(46, 4, 6), palette_mid=(206, 20, 26),
            palette_bright=(255, 94, 94), accent=(255, 40, 40))
_VIOLET = dict(palette_dark=(20, 8, 10), palette_mid=(84, 30, 38),
               palette_bright=(156, 84, 90), accent=(168, 62, 70))
...
EMOTIONS: dict[str, EmotionProfile] = {
    ...
    # ── Mark XXVI expression set (§8) — micro-expression scale, not cartoon.
    # CONCENTRATING — deeper than thinking: maximal condensed density, brows
    # lowered and steady, eyes narrowed, minimal drift. The look of held focus.
    "concentrating": EmotionProfile(
        "concentrating", voxel_density=1.2, brow=-0.55, eye_width=0.9, eye_height=0.52,
        mouth_curve=-0.02, mouth_tension=0.25, glow=0.5, glow_pulse=0.0,
        speed=1.0, particle_velocity=0.5, particle_direction="drift",
        turbulence=0.03, **_DEEP_BLUE),
    # CURIOUS — brows up, eyes widened, a small upturn; brighter, lively drift.
    "curious": EmotionProfile(
        "curious", voxel_density=0.98, brow=0.45, eye_width=1.12, eye_height=1.05,
        mouth_curve=0.18, mouth_tension=0.0, glow=0.72, glow_pulse=0.05,
        speed=1.05, particle_velocity=1.1, particle_direction="up",
        turbulence=0.2, **_BRIGHT_BLUE),
    # AMUSED — a relaxed half-smile, brows softly raised, warm and steady.
    "amused": EmotionProfile(
        "amused", voxel_density=0.95, brow=0.28, eye_width=1.05, eye_height=0.95,
        mouth_curve=0.42, mouth_tension=0.0, glow=0.8, glow_pulse=0.08,
        speed=1.05, particle_velocity=1.15, particle_direction="up",
        turbulence=0.22, **_BRIGHT_BLUE),
    # CONFUSED — a single-note furrow (symmetric approx), eyes slightly narrowed,
    # mouth faintly tense; the drift hesitates.
    "confused": EmotionProfile(
        "confused", voxel_density=1.05, brow=-0.22, eye_width=0.96, eye_height=0.72,
        mouth_curve=-0.06, mouth_tension=0.22, glow=0.55, glow_pulse=0.0,
        speed=0.85, particle_velocity=0.6, particle_direction="drift",
        turbulence=0.12, **_BLUE),
    # UNCERTAIN — softer than confused: gentle inner-brow, lowered eyes, low glow.
    "uncertain": EmotionProfile(
        "uncertain", voxel_density=0.98, brow=-0.1, eye_width=0.98, eye_height=0.62,
        mouth_curve=-0.12, mouth_tension=0.12, glow=0.45, glow_pulse=0.0,
        speed=0.8, particle_velocity=0.55, particle_direction="drift",
        turbulence=0.08, **_VIOLET),
    # DISAPPOINTED — inner-raised brows, drooped eyes, downturned mouth; dim,
    # slow. Distinct from sadness by holding more density (composure).
    "disappointed": EmotionProfile(
        "disappointed", voxel_density=0.95, brow=0.12, eye_width=0.94, eye_height=0.55,
        mouth_curve=-0.38, mouth_tension=0.08, glow=0.4, glow_pulse=0.0,
        speed=0.72, particle_velocity=0.5, particle_direction="down",
        turbulence=0.05, **_VIOLET),
    # PROUD — brows up, eyes open, a warm confident smile; high steady glow.
    "proud": EmotionProfile(
        "proud", voxel_density=1.0, brow=0.5, eye_width=1.08, eye_height=1.0,
        mouth_curve=0.48, mouth_tension=0.0, glow=0.9, glow_pulse=0.06,
        speed=1.0, particle_velocity=1.2, particle_direction="up",
        turbulence=0.18, **_BRIGHT_BLUE),
    # REASSURING — a calm, gentle smile; softened eyes; slow, steady, warm.
    "reassuring": EmotionProfile(
        "reassuring", voxel_density=1.0, brow=0.22, eye_width=1.02, eye_height=0.85,
        mouth_curve=0.35, mouth_tension=0.0, glow=0.68, glow_pulse=0.03,
        speed=0.82, particle_velocity=0.75, particle_direction="up",
        turbulence=0.08, **_BLUE),
    # EMPATHETIC — warm concern: inner-brow lift, soft eyes, a faint gentle
    # downturn, low warm glow. Present WITH the user, not alarmed for them.
    "empathetic": EmotionProfile(
        "empathetic", voxel_density=1.0, brow=0.18, eye_width=1.0, eye_height=0.8,
        mouth_curve=-0.08, mouth_tension=0.05, glow=0.6, glow_pulse=0.0,
        speed=0.8, particle_velocity=0.6, particle_direction="drift",
        turbulence=0.06, **_AMBER),
}
