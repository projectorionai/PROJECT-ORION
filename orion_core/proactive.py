...
#: A date, optionally with a time: "2026-09-24", "2026-09-24 08:38",
#: "2026-09-24T08:38:00". Exact, so a following "08:38: Lab" colon is not
#: swallowed into the time (which printed as "at 2026-09-24 08:38:.").
_STAMP = r"\d{4}-\d{2}-\d{2}(?:[ T]\d{1,2}:\d{2}(?::\d{2})?)?"
...
class ProactiveIntelligence:
    ...
    def __init__(
        ...
        # Attached after construction (built later in app.py). Tasks the user
        # removed there are never raised again from Notion either.
        self.cognition: Any | None = None
    ...
    async def check_now(self) -> ToolResult:
        ...
        if not suggestions:
            return ToolResult("Nothing pressing. Your desk is clear.")
    ...
    async def _check_tasks_and_calendar(self) -> list[Suggestion]:
        ...
        try:
            ...
            dismissed = getattr(self.cognition, "is_dismissed", None)
            for title, due in self._parse_dated_rows(tasks.text):
                if dismissed is not None and dismissed(title):
                    continue
                hours = self._hours_until(due)
                if hours is None:
                    continue
                if hours < 0:
                    # Keywords, not positions: the fields run key, TEXT,
                    # salience — positionally this made the text "3" (the
                    # log said "PROACTIVE: 3") and the salience a sentence,
                    # so ranking them crashed the survey.
                    out.append(Suggestion(
                        key=f"task_overdue:{title}", salience=3,
                        text=f"Task overdue: '{title}' was due {abs(hours):.0f}h ago.",
                        at=time.monotonic()))
                elif hours <= 48:
                    sal = 3 if hours <= 12 else 2
                    out.append(Suggestion(
                        key=f"task_due:{title}", salience=sal,
                        text=f"'{title}' is due in {hours:.0f} hours.",
                        at=time.monotonic()))
        except Exception:
            pass
        try:
            ...
            for title, when in self._parse_dated_rows(events.text):
                ...
                if hours is not None and 0 <= hours <= 24:
                    out.append(Suggestion(
                        key=f"event:{title}", salience=2,
                        text=f"Today's schedule: '{title}' at {when}.",
                        at=time.monotonic()))
        except Exception:
            pass
        return out
    ...
    async def _check_workflow_patterns(self) -> list[Suggestion]:
        """Offer to name a run of tools the user keeps repeating.

        Salience 1 deliberately: this is an "if you like" observation about
        how they work, not something that needs attention, so it stays out of
        the banner and is never spoken. Suggesting only — accepting it means
        defining a workflow through the normal tool, with real arguments the
        detector doesn't retain.
        """
        ...
        for seq in sequences[:2]:
            suggestions.append(Suggestion(
                key=f"workflow_pattern:{seq.describe()}",
                salience=1,
                text=(f"You've run {seq.describe()} {seq.occurrences} times — "
                      f"shall I save that as a workflow called "
                      f"'{seq.suggested_name()}'?"),
                at=time.monotonic(),
            ))
        return suggestions
    ...
    def _parse_dated_rows(self, text: str) -> list[tuple[str, str]]:
        ...
        for line in text.splitlines():
            ...
            if not line:
                continue
            due_match = re.search(r"\bdue\s+(" + _STAMP + ")", line, re.IGNORECASE)
            if due_match:
                # The title is what comes BEFORE "due …" — a line with no
                # "[status]" tag used to keep the date in its title.
                title = re.split(r"\s*\[|\s{2,}", line[:due_match.start()])[0]
                rows.append((title.strip(" -—:,")[:60], due_match.group(1).strip()))
                continue
            date_match = re.match("(" + _STAMP + r")\s*:?\s*(.+)", line)
            if date_match:
