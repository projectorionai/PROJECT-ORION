...
class KnowledgePackManager:
    ...
    def remove(self, pack_id: str) -> ToolResult:
        ...
        if pack is None:
            return ToolResult(f"No installed pack '{pack_id}'.", ok=False)
        # The file goes FIRST. This used to drop the pack from memory and then
        # swallow a failed delete, so it reported "Removed" while the file
        # survived — and _load_from_disk() reinstalled it on the next start.
        # If the file cannot be removed, the pack stays installed and says so.
        try:
            (PACKS_DIR / f"{pack_id}.json").unlink(missing_ok=True)
        except OSError as exc:
            return ToolResult(
                f"Couldn't remove knowledge pack '{pack.title}' — its file is in "
                f"use or protected ({first_line(exc, 60)}). It is still installed.",
                ok=False)
    ...
    def _persist(self, pack: KnowledgePack) -> None:
        try:
            ...
            atomic_write_text(PACKS_DIR / f"{pack.id}.json",
