...
from .executors import run_bulk
...
class LearningService:
    ...
    async def learn(self, source: str, topic: str = "") -> ToolResult:
        ...
        if not source:
            return ToolResult("What would you like me to learn?", ok=False)
        try:
        ...
        if not text or len(text.strip()) < 20:
            return ToolResult(
                f"I couldn't get enough text from that {kind}.", ok=False)

        topic = topic.strip() or self._infer_topic(text, source)
        facts = await self._distil(text, topic)
        if not facts:
            return ToolResult("I read it but couldn't extract anything worth keeping.", ok=False)
        ...
        return ToolResult(
            f"Learned and committed to memory — {stored} fact(s) on '{topic}' "
    ...
    async def recall(self, query: str) -> ToolResult:
        ...
        if not query:
            return ToolResult("What shall I recall?", ok=False)
        ...
        if not hits:
            return ToolResult(f"I haven't learned anything matching '{query}' yet.")
        return ToolResult("Here's what I know:\n" + "\n".join(f"- {h}" for h in hits if h))
    ...
    async def learn_folder(self, folder: str, topic: str = "", recursive: bool = True,
        ...
        if not root.is_absolute():
            root = BASE_DIR / root
        if not root.is_dir():
            return ToolResult(f"That folder doesn't exist: {root}", ok=False)
        ...
        for index, path in enumerate(files, 1):
            # §5.3: bulk file reads/extraction/storage run on the bounded bulk
            # pool (run_bulk), never the default executor — so a large folder
            # ingest can't add head-of-line latency to audio/voice work.
            try:
                text = await run_bulk(self._read_file, path, 16000)
            except Exception:
                text = ""
            ...
            if deep and self.router.has_text_fallback():
                ...
                facts = await run_bulk(self._extractive, text)
            if not facts:
                continue
            file_slug = re.sub(r"[^a-z0-9]+", "_", path.stem.lower()).strip("_")[:36] or "doc"
            stored = await run_bulk(
        ...
        return ToolResult(
            f"Ingested '{base_topic}' — {total_facts} fact(s) committed from "
    ...
    async def correct(self, topic: str, correction: str) -> ToolResult:
        ...
        if not correction:
            return ToolResult("What's the correction?", ok=False)
        try:
        ...
        self.bus.dashboard_event.emit("learned", {
            ...
            "source": "correction"})
        return ToolResult(
            f"Understood — I've corrected that and will treat it as authoritative"
    ...
    async def forget(self, query: str) -> ToolResult:
        ...
        query = str(query or "").strip()
        if not query:
            return ToolResult("What would you like me to forget?", ok=False)
        ...
        if removed:
            ...
            return ToolResult(
                f"Done — I've forgotten {removed} learned record(s) relating to "
                f"'{query}'. Curated built-in knowledge is untouched.")
        return ToolResult(
            f"I found nothing I'd learned about '{query}' to forget (built-in "
