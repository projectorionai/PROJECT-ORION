...
class AgencyPipelineService:
    ...
    def create_campaign(self, name: str, brand: str = "", value: float = 0.0,
        ...
        if not name:
            return ToolResult("A campaign needs a name.", ok=False)
        cid = self._slug(name)
        ...
        self._commit()
        return ToolResult(f"Campaign '{name}' created at the Lead stage.")
    ...
    def update_stage(self, ref: str, stage: str) -> ToolResult:
        ...
        self._commit()
        return ToolResult(f"'{camp.name}' moved from {old} → {target.value}.")
    ...
    def schedule_content(self, ref: str, title: str, platform: str = "",
        ...
        if camp is None:
            return self._not_found(ref)
        if not title.strip():
            return ToolResult("A content title is required.", ok=False)
        ...
        camp.updated = utc_stamp()
        self._commit()
        return ToolResult(f"Content '{title}' added to '{camp.name}' ({status}).")
    ...
    def log_performance(self, ref: str, metric: str, value: float) -> ToolResult:
        ...
        return ToolResult(
            ...
            f"(total engagement {camp.engagement_total():,.0f})."
        )
    ...
    def delete_campaign(self, ref: str, confirm: bool = False) -> ToolResult:
        ...
        if not confirm:
            return ToolResult(
                ...
                + " is permanent. Confirm and I shall remove it.",
                ok=False,
            )
        ...
        return ToolResult(f"Deleted the campaign '{name}'.")

    def _not_found(self, ref: str) -> ToolResult:
        known = ", ".join(c.name for c in self._campaigns.values()) or "none yet"
        return ToolResult(f"No campaign matches '{ref}'. Known campaigns: {known}.", ok=False)
    ...
    def snapshot_text(self) -> ToolResult:
        ...
        return ToolResult("\n".join(lines) if len(lines) > 1
                          else "The pipeline is empty. Create a campaign to begin.")
    ...
    def _save(self) -> None:
        try:
            ...
            atomic_write_text(PIPELINE_FILE, json.dumps(payload, indent=2), encoding="utf-8")
