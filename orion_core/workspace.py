...
from .app_context import describe_context
...
class WorkspaceSnapshot:
    ...
    def summary(self) -> str:
        return (
            ...
            + (f"; {self.active_context}" if self.active_context else "")
        )
...
class WorkspaceManager:
    ...
    def _capture_sync(self) -> WorkspaceSnapshot:
        ...
        for w in windows:
            ...
            if w.kind == "browser":
                tab = re.sub(r"\s*[-–—]\s*(Google Chrome|Microsoft.Edge|Mozilla Firefox|Opera).*$",
                             "", w.title).strip()
                if tab:
        ...
        return WorkspaceSnapshot(
            ...
            active_context=describe_context(active),
            windows=windows,
