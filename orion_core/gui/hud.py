...
depth-shaded sphere wrapped in a crimson Fresnel rim, breathing at its
...
def _rgba(rgb: tuple[int, int, int], alpha: float) -> QColor:
    """Build a QColor from an (r, g, b) tuple — every channel clamped to int
    (eased/mixed palettes arrive as floats, and PyQt6's QColor rejects floats,
    which aborts the app from inside paintEvent)."""
...
class CentralHud(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self.timer.start()
        # 7.04 ms of QPainter work per frame, measured — and under qasync this
        # runs ON the asyncio loop, so it is time the audio callback does not
        # get. Most of those frames redraw an identical picture; the budget
        # spends them only while something is actually moving.
        from .animation_budget import AnimationBudget
        self._budget = AnimationBudget(self.timer)
    ...
    def _tick(self) -> None:
        ...
        # Particles are motion in their own right, so their count keeps the
        # widget "active" even when every eased channel has settled.
        if self._budget.note((
                self.amplitude, self.face_amount, self.iris,
                self.banner_alpha, float(len(self.particles)))):
            self.update()
    ...
    def _paint_starfield_static(self, painter: QPainter, width: int, height: int) -> None:
        ...
        for _ in range(int(width * height / 4200)):
            ...
            if twinkle > 0.82:                       # rare accent star
    ...
    def _paint_ambient_static(self, painter: QPainter, width: int, height: int) -> None:
        ...
        # Faint rim-light bloom, offset — gives the scene a cold key light.
        # Neutral rather than cyan: against crimson a neutral already reads
        # cold, and it keeps the interface to one hue.
        cool = QRadialGradient(width * 0.66, height * 0.62, min(width, height) * 0.42)
        cool.setColorAt(0.0, QColor(*C.SILVER_RGB, 16))
        cool.setColorAt(1.0, QColor(*C.SILVER_RGB, 0))
    ...
    def _paint_brackets_static(self, painter: QPainter, width: int, height: int) -> None:
        """Corner targeting brackets with a neutral inner tick — cached."""
        margin = 22
        ...
        for (px, py), (sx, sy) in corners:
            ...
            painter.setPen(QPen(QColor(*C.SILVER_RGB, 130), 1))
    ...
    def _paint_scanline(self, painter: QPainter, width: int, height: int) -> None:
        ...
        grad.setColorAt(0.0, QColor(*C.SILVER_RGB, 0))
        grad.setColorAt(0.5, QColor(*C.SILVER_RGB, 20))
        grad.setColorAt(1.0, QColor(*C.SILVER_RGB, 0))
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(grad)
        painter.drawRect(QRectF(0.0, y - band, float(width), band * 2))
        painter.setPen(QPen(QColor(*C.SILVER_RGB, 38), 1))
