...
class FocusTimer(QFrame):
    def __init__(self, bus: OrionBus) -> None:
        ...
        self.clock.setStyleSheet(
            f"font-size:32px;font-weight:800;color:{C.PRI_HI};")
        self.phase = QLabel("Focus block — ready when you are.")
    ...
    def _reset(self) -> None:
        ...
        self.phase.setText("Focus block — ready when you are.")
    ...
    def _tick(self) -> None:
        ...
        if self._remaining <= 0:
            if not self._on_break:
                ...
                self.bus.speak_request.emit(f"Focus block complete{note}. That's {self._sessions} today. Time for a short break.")
            else:
                self.bus.speak_request.emit("Break over. Back to it.")
            ...
            self.phase.setText(f"Break — step away. ({self._sessions} done)"
...
class MarginCalculator(QFrame):
    def __init__(self) -> None:
        ...
        self.out = QLabel("Enter a cost and price."); self.out.setObjectName("mutedLabel")
    ...
    def _calc(self) -> None:
        ...
        self.out.setText("\n".join(lines) or "Enter a cost and price.")
...
class AdCalculator(QFrame):
    def __init__(self) -> None:
        ...
        for w in (self.spend, self.revenue, self.orders, self.margin):
            w.textChanged.connect(self._calc); layout.addWidget(w)
        self.out = QLabel("Enter spend and revenue."); self.out.setObjectName("mutedLabel")
        self.out.setWordWrap(True); layout.addWidget(self.out)
    ...
    def _calc(self) -> None:
        ...
        if spend <= 0:
            self.out.setText("Enter spend and revenue."); return
        roas = rev / spend
...
class CurrencyConverter(QFrame):
    ...
    def __init__(self, bus: OrionBus) -> None:
        ...
        swap = describe_control(
            QPushButton("⇄"), "Swap currencies", "Exchange the from and to currencies")
        swap.setFixedWidth(38); swap.clicked.connect(self._swap)
    ...
    async def _do(self) -> None:
        ...
        try:
            ...
            self.out.setText("Rate unavailable offline — reconnect for live currency.")
...
class WorldClock(QFrame):
    ...
    def __init__(self) -> None:
        ...
        self._timer = QTimer(self); self._timer.setInterval(1000); self._timer.timeout.connect(self._tick)
        # _render_clocks, not _tick: the priming paint must bypass the
        # visibility guard, since nothing is visible yet at construction.
        self._timer.start(); self._render_clocks()

    def _tick(self) -> None:
        """Timer target — idle while the TOOLKIT page is not the visible one."""
        if not self.isVisible():
            return
        self._render_clocks()

    def _render_clocks(self) -> None:
...
class IdeaCapture(QFrame):
    ...
    def _save(self) -> None:
        ...
        try:
            ...
            # The idea was NOT saved. Adding it to the list and logging
            # "captured" anyway told the user it had been kept, and the text
            # was already cleared from the box — so the only copy was gone.
            # Leave the input alone so nothing is lost, and say what happened.
            self.list.insertItem(0, f"⚠ NOT SAVED — {idea}")
            self.bus.log.emit(
                f"IDEA: could not save — {first_line(exc, 120)}. "
                "The text is still in the box; try again."
            )
            return
        self.input.clear()
...
class DeadlineCountdown(QFrame):
    ...
    def _add(self) -> None:
        ...
        try:
            ...
            pass
        self._render_list()   # the user just acted — show it now, guard or not
    ...
    def _load(self) -> None:
        try:
            ...
            pass
        # _render_list, not _render: _load runs from __init__, before anything
        # is visible, so the guarded path would leave the list blank for 30s.
        self._render_list()
...
class RemindersWidget(QFrame):
    def __init__(self, bus: OrionBus, reminders: Any) -> None:
        ...
        # _refresh_list, not _refresh: the priming paint must bypass the
        # visibility guard, since nothing is visible yet at construction.
        self._timer.start(); self._refresh_list()
    ...
    def _add(self) -> None:
        ...
        self.text.clear(); self.mins.clear()
        self._refresh_list()   # the user just acted — show it now, guard or not

    def _clear(self) -> None:
        self.reminders.cancel(); self._refresh_list()
...
class ProtocolsWidget(QFrame):
    def __init__(self, bus: OrionBus, protocols: Any) -> None:
        ...
        self.out = QLabel("Select and engage a protocol."); self.out.setObjectName("mutedLabel")
        self.out.setWordWrap(True); layout.addWidget(self.out)
    ...
    def _run(self) -> None:
        ...
        # Held until it finishes: a discarded task is weakly referenced and
        # can be collected part-way through, leaving the protocol half-run
        # and the panel showing nothing.
        from ..tasks import spawn

        spawn(_go(), bus=getattr(self, "bus", None), label="protocol run")
