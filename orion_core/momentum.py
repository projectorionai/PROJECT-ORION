...
class MomentumEngine:
    ...
    def focus(self) -> ToolResult:
        ...
        if not tasks:
            ...
            return ToolResult(
                f"No open tasks are tracked{hint}. Tell me the goal and I'll break it "
        ...
        lines = [f"Next action{(' on ' + project) if project else ''}: "
    ...
    def standup(self) -> ToolResult:
        ...
        if not tasks and not projects and not priorities:
            return ToolResult(
                "Nothing is tracked yet. Name a project and its goal and I'll set up "
                "milestones and next actions so we can start shipping.")
        ranked = self._rank_tasks(tasks)
        overdue = [t for t in ranked if self._is_overdue(t.get("due", ""))]
        lines = ["Shipping stand-up:"]
        if projects:
    ...
    async def plan(self, project: str = "", goal: str = "") -> ToolResult:
        ...
        if not project and not goal:
            return ToolResult("Which project or goal shall I plan?", ok=False)
        ...
        if self.router is not None and self.router.has_text_fallback():
            ...
            try:
                ...
                if answer.strip():
                    ...
                    return ToolResult(f"Shipping plan for '{subject}':\n\n{answer.strip()}")
            except Exception as exc:
                self.bus.log.emit(f"MOMENTUM: model plan failed - {first_line(exc, 80)}")

        # Offline template — still actionable.
        template = (
            f"Shipping plan for '{subject}' (offline template — I'll sharpen it when a model is reachable):\n"
